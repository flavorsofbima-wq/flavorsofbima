...
META = {
  ...
  "vellulli-karam-podi": {"emoji":"🧄","gradient":"linear-gradient(145deg,#14100a,#3a3010,#6a5820)","shortDesc":"Bold garlic karam podi.","description":"An intensely flavoured garlic podi — pungent, rich and bold. A little goes a long way over rice, idly or dosa.","ingredients":["Garlic","Red chilli","Urad dal","Salt"],"badge":""},
...
def main():
    ...
    for row in ws.iter_rows(min_row=6, values_only=True):
        ...
        # Description & ingredients come from the Excel (per-product).
        # Blank cell -> fall back to built-in META text.
        import re as _re
        xl_desc = str(row[idx["description"]]).strip() if ("description" in idx and row[idx["description"]]) else ""
        xl_ing_raw = str(row[idx["ingredients"]]).strip() if ("ingredients" in idx and row[idx["ingredients"]]) else ""
        xl_ingredients = None
        if xl_ing_raw:
            parts = _re.split(r"\r?\n|,", xl_ing_raw)
            xl_ingredients = [_re.sub(r"^[\s•\-\*\u2022]+", "", p).strip() for p in parts]
            xl_ingredients = [p for p in xl_ingredients if p]
        description = xl_desc or meta.get("description", "")
        ingredients = xl_ingredients if xl_ingredients else meta.get("ingredients", [])
        short_desc = (_re.split(r"(?<=\.)\s", xl_desc)[0].strip() if xl_desc else meta.get("shortDesc", ""))
        shelf_life = str(row[idx["shelf_life"]]).strip() if ("shelf_life" in idx and row[idx["shelf_life"]]) else ""
        availability_note = str(row[idx["availability_note"]]).strip() if ("availability_note" in idx and row[idx["availability_note"]]) else ""
